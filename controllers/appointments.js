...
exports.addAppointment = async (req, res, next) => {
    try {
        ...
        if (existedAppointments.length >= 3 && req.user.role !== 'admin') return res.status(400).json({
            success: false,
...
exports.updateAppointment = async (req, res, next) => {
    try {
        ...
        if (appointment.user.toString() !== req.user.id && req.user.role !== 'admin') return res.status(401).json({
            success: false,
